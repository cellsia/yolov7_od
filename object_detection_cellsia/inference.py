...
def get_names_colors(model):
    ...
    return names, colors, true_colors, class_colors
        ...
        for i, det in enumerate(pred):
            if len(det):  # Si hay detecciones
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    plot_one_box(xyxy, im0s, color=colors[predicted_class], line_thickness=2)
            '''
...
def main(input_dir, img_size, model, device, half, conf_thres, iou_thres, classes, augment, names, colors, data_config, weights, output_dir, key, class_colors, max_examples=20):
    ...
    for img_path in image_paths:
        ...
        image_examples = procesar_detecciones(pred, img, im0s, names, colors, txt_path, processed_images_dir, path, image_examples, expected_classes)
...
if __name__ == "__main__":
    ...
    names, colors, true_colors, class_colors = get_names_colors(model)
