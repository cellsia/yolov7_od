...
class EarlyStopping:
    def __init__(self, save_dir, patience, verbose=True, delta=0, start_epoch=100):
    ...
    def __call__(self, metric, model, epoch):
        ...
        score = metric
        self.logger.info(f'Evaluating early stopping at epoch {epoch}, current metric: {score:.6f}')
        sys.stdout.flush()

        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(metric, model)
            self.logger.info(f'Best metric initialized: {score:.6f}')
        elif score < self.best_score + self.delta:
            self.counter += 1
            self.logger.warning(f'Early stopping counter: {self.counter}/{self.patience} '
    ...
    def save_checkpoint(self, metric, model):
        if self.verbose:
            self.logger.info(f'Saving model (metric: {metric:.6f} > {self.metric_max:.6f})')
        self.metric_max = metric
        save = self.save_dir / 'weights' / 'checkpoint.pt'
        torch.save(model.state_dict(), save)
        self.logger.info(f'Model saved to {save}')
        sys.stdout.flush()

def train(hyp, opt, device, tb_writer=None):
    logger = logging.getLogger('train_early')
    #logger.info(colorstr('Epoch hyperparameters: ') + ', '.join(f'{k}={v}' for k, v in hyp.items()))
    ...
    early_stopping = EarlyStopping(save_dir, patience, verbose=True)
