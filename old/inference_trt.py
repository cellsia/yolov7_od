...
class TensorRTModel:
    ...
    def infer(self, img):
        ...
        return self.outputs[0]['host'].reshape(-1, 85)  # Ajusta según tu modelo

def load_model(engine_path):
    return TensorRTModel(engine_path)
...
def realizar_inferencia_trt(img, model, conf_thres, iou_thres, classes, agnostic_nms):
    preds = model.infer(img)
    preds = torch.tensor(preds)  # Convertir a tensor para usar con `non_max_suppression`
...
def main(input_dir, model, input_name, output_name, img_size,conf_thres, iou_thres, classes, augment, names, data_config, weights, output_dir, key, class_colors, max_examples=20):
    ...
    for img_path in image_paths:
        ...
        pred = realizar_inferencia_trt(img, model, args.conf_thres, args.iou_thres, None, agnostic_nms=False)  
